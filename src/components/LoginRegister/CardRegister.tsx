        ...
        .then(
          async (response: any) => {
            ...
            if (userData) {
              ...
              // )
              notifyUser( 'Registration successful. Please verify your email to access new features', 'Please wait...', 'success', '/login')
              ...
              // handleError('An error occurred: Possible causes => 1. user may already exist. 2. Phone number not correct 3. Passwords Do not match')
               notifyUser( 'User not found. Please sign up', 'Please wait...', 'error', 'reload')
            }
          }
        )
        .catch((error: any) => {
          console.log()
          notifyUser(`Opps! registration failed`, `${error}`, "error", "error")
          // handleError(
